...
struct NodeIndexUnit {
	...
	int*** backtrack_index = NULL; //backtrack_index[i][j] = candidates of this unit when the i-th neighbor, regarding DAG, mapped to the j-th candidate of the neighbor.
	int** backtrack_size_of_index = NULL; //backtrack_size_of_index[i][j] = size of back_track_index[i][j]. That is, the number of candidates of this unit when the i-th neighbor mapped to the j-th candidate of the neighbor.
	int* backtrack_neighbor_cand_size = NULL; //backtrack_neighbor_cand_size[i] = the number of candidates of the i-th neighbor.
...
bool order_flag = false; //set use_candidate_size_order in readAndProcessDataGraph when order_flag turns false. (order_flag turns true when program argument specifies matching order)
...
int * nodes_info = NULL; // this array stores the start position of each node's adjacent list in the "nodes_data"
pair<int, int> * nodes_to_label_info = NULL; // given a node, and a label, it can extract the children of the given node with the given label
...
int node_degree_query[MAX_QUERY_NODE]; //store the degree of each node in the query graph
int core_number_query[MAX_QUERY_NODE];
int query_nodes_array_info[MAX_QUERY_NODE + 1];
int query_neighbor_index[MAX_QUERY_NODE][MAX_QUERY_NODE]; // j-th node is query_neighbor_index[i][j]-th neighbor of i-th node
//coreDecomposition
...
int* DAG_parent_query[MAX_QUERY_NODE]; //DAG_parent_query[i] contains parent nodes, regarding DAG of query graph, of i-node.
int* DAG_child_query[MAX_QUERY_NODE];  //DAG_child_query[i] contains child nodes, regarding DAG of query graph, of i-node.
int* DAG_child_query_parent_index[MAX_QUERY_NODE]; //DAG_child_query_parent_index[i][j] contains parent, which is i-th node, index of the j-th child node of i-node. It is used for ajacency list prunning.
int DAG_parent_query_size[MAX_QUERY_NODE]; //DAG_parent_query_size[i] contains the number of parent nodes, regarding DAG of query graph, of i-node.
int DAG_child_query_size[MAX_QUERY_NODE];  //DAG_child_query_size[i] contains the number of child nodes, regarding DAG of query graph, of i-node.
bit_vector DAG_ancestor[MAX_QUERY_NODE];
bit_vector adaptive_ancestor[MAX_QUERY_NODE][MAX_QUERY_NODE];
int* label_frequency_rank;  //label_frequency_rank[i] contains label i's rank in terms of the label_frequency in increasing order; i.e., label_frequency_rank[0] has the lowest label_frequency.
...
int position[MAX_QUERY_NODE]; // position before pop
int frontier_index[MAX_QUERY_NODE]; // store index of i-th vertex of query graph, -1 if i-th vertex is not an extendable vertex
int global_matching_order[MAX_QUERY_NODE];
...
int * * sum_nec_cands_array; //added for the optimization of performing all MM first
int * size_sum_nec_cands;	 //added for the optimization of performing all MM first
int  * idx_v_cands; //length: cnt_node. its each entry stores the number of query nodes contain the corresponding candidate.
vector < pair <int, int> > * v_cands_info; // a combination of v_cands and v_cands_pos
pair <int, int>  * u_cand_info; //two-d array:
								//[i,j] means the j-th candidate pair of the i-th nec node(in the nec-node categorized by label)
								//each candidate pair stores a candidate and the position of "the i-th nec node" in this candidate's u_cand set.

int * flag_sum_nec_cands; //indicate whether or not a candidate has been added into the candidate already.
...
int combination_array[MAX_QUERY_NODE];
int combination_array_index = 0;
pair <int, int>  v_nec_count[MAX_QUERY_NODE]; // used in the map special tree function
...
int mapped_neighbors[MAX_QUERY_NODE];
int* temp_intersection_array[MAX_QUERY_NODE][MAX_QUERY_DEGREE - 1];
int temp_intersection_array_index[MAX_QUERY_NODE][MAX_QUERY_DEGREE - 1];
...
//The following three are the temporary arrays for the indexSet element, used during the global candidate exploration
...
inline bool sortLLCnode(pair<int, pair<int, double> > a, pair<int, pair<int, double> > b) {// the second pair is <node sum, cand sum>
...
inline bool sort_by_effective_cand_size_pair(pair<int, int> p1, pair<int, int> p2) {
...
inline bool sortByLabelFrequencyRank_Query_inc(const int id1, const int id2) {
	return (label_frequency_rank[nodes_label_query[id1]] < label_frequency_rank[nodes_label_query[id2]]);
}
...
void getLimit_full(string str_full_limit, long long & LIMIT) {
...
inline void parametersInitilisingBeforeQuery() {
	...
	u_cand_MM = new int[MAX_QUERY_NODE * max_label_counter]; //definite upper bound]
	...
	dist = new int[MAX_QUERY_NODE + 1];

	u_cand_info = new pair<int, int>[MAX_QUERY_NODE * max_label_counter]; //definite upper bound]
	...
	for (int i = 0; i < cnt_node; i++)
		v_cands_info[i].resize(MAX_QUERY_NODE);
	...
	NEC_mapping_Actual = new int[(cnt_unique_label + 1) * MAX_QUERY_NODE]; //label and the parent node id
	memset(NEC_mapping_Actual, 0, sizeof(int) * (cnt_unique_label + 1) * MAX_QUERY_NODE);
	...
	for (int i = 0; i < MAX_QUERY_NODE; i++) {
		...
		indexSet[i].vis = new bool[max_label_counter];
		for (int j = 0; j < MAX_QUERY_DEGREE - 1; j++) {
			temp_intersection_array[i][j] = new int[max_label_counter];
		}
	}
    ...
    chosen_vertex = new bool[cnt_node];
